...
///   - 70% buy (ETH -> LINEASTR), 30% sell (LINEASTR -> ETH)
///   - buy size: 0.0008-0.0015 ETH (random within range)
///   - sell size: 200-800 LINEASTR (random within range, capped to caller's balance)
///
/// USAGE:
...
contract MarketSimRound is Script {
    ...
    function run() external {
        ...
        LineastrTestSwapper swapper = LineastrTestSwapper(payable(SWAPPER));
        ...
        console.log("Caller LINEASTR (wei):", callerBal);
        console.log("currentFees pre (wei):", proxy.currentFees());

        // Decide: 70% buy, 30% sell. Force buy if caller has < 200 LINEASTR (can't sell meaningfully).
        ...
        if (doBuy) {
            ...
            // Sell size: 200-800 LINEASTR (in 18-decimal units), capped to balance / 10
            ...
            console.log("ACTION: SELL (LINEASTR wei):", amount);
            // Approve swapper to pull LINEASTR (one-shot allowance for this round)
            (bool ok,) = LINEASTR_PROXY.call(abi.encodeWithSignature("approve(address,uint256)", SWAPPER, amount));
        ...
        console.log("Caller LINEASTR post (wei):", proxy.balanceOf(caller));
    }
}
